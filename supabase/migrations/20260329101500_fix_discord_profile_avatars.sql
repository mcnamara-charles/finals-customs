-- Ensure profile avatar_url is populated for OAuth providers (Discord can use `picture` or id/hash fields).

do $$
begin
  update public.profiles p
  set
    avatar_url = src.resolved_avatar_url,
    updated_at = now()
  from (
    select
      u.id as user_id,
      coalesce(
        ...
        case
          when nullif(trim(u.raw_user_meta_data ->> 'provider_id'), '') is not null
            and nullif(trim(u.raw_user_meta_data ->> 'avatar'), '') is not null
          then
            'https://cdn.discordapp.com/avatars/'
            || trim(u.raw_user_meta_data ->> 'provider_id')
            || '/'
            || trim(u.raw_user_meta_data ->> 'avatar')
            || '.png'
          else null
        end
  ...
  where p.user_id = src.user_id
    ...
    and src.resolved_avatar_url is not null;
end $$;
