-- Fix username derivation for Discord OAuth users.
-- Prefer Discord identity fields over email local-part, and backfill affected existing users.
...
begin
  for r in
    ...
    where
      discord_slug is not null
      and current_username <> discord_slug
      and (
        current_username = coalesce(email_slug, '')
        or current_username = 'member'
        or current_username ~ '^member_[0-9]+$'
      )
    order by created_at nulls first, user_id
  loop
    candidate := r.discord_slug;
    suffix := 1;
    while exists (
      select 1
      ...
      suffix := suffix + 1;
      candidate := r.discord_slug || '_' || suffix::text;
    end loop;
